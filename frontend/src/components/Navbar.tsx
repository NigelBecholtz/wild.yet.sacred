    ...
    <nav ref={navRef} className="fixed top-0 w-full z-50 bg-surface/80 backdrop-blur-md border-b border-surface-container-high/30 shadow-[0_20px_50px_rgba(59,12,12,0.15)]">
      <div className="flex justify-between items-center px-4 sm:px-6 lg:px-8 py-4 max-w-7xl mx-auto">
          ...
          {navLinks.map(({ to, label }) => (
            <Link
              ...
              className={`font-headline antialiased tracking-tight transition-colors duration-300 ${
                isActive(to)
                  ? 'text-on-surface border-b border-secondary-container pb-1'
                  : 'text-on-surface/70 hover:text-secondary-container'
              }`}
            >
              {label}
            </Link>
        ...
        <div className="flex items-center gap-3 sm:gap-5">
          ...
          <button
            onClick={toggleLang}
            className="hidden sm:block text-on-surface/50 text-xs uppercase tracking-widest hover:text-secondary-container transition-colors"
          >
          ...
          {isAuthenticated ? (
            <div className="relative hidden md:block">
              <button
                ...
                className="text-secondary-container font-label text-sm tracking-widest uppercase hover:opacity-80 transition-opacity"
              >
                {user?.name?.split(' ')[0]} ▾
              </button>
              {userMenuOpen && (
            ...
            <div className="hidden md:flex items-center gap-4">
              <Link
                to="/login"
                className="text-on-surface/70 hover:text-secondary-container transition-colors text-sm font-label uppercase tracking-widest"
              >
                {t('nav.login')}
              </Link>
              <Link
      ...
      >
        <div className="bg-surface/97 backdrop-blur-xl border-t border-outline-variant/20 px-6 py-6 flex flex-col">
          ...
          {navLinks.map(({ to, label }) => (
            <Link
              ...
              className={`font-headline text-lg py-3 border-b border-outline-variant/10 last:border-0 transition-colors ${
                isActive(to) ? 'text-on-surface' : 'text-on-surface/70 active:text-secondary-container'
              }`}
            >
              {label}
            </Link>
          ...
          <div className="pt-6 mt-2 border-t border-outline-variant/20 flex flex-col gap-3">
            {isAuthenticated ? (
              <>
                <p className="text-on-surface-variant font-label text-xs tracking-widest uppercase opacity-60 pb-1">
                  {user?.name}
                </p>
                {user?.isAdmin && (
                  <Link
              ...
              <div className="flex flex-col gap-3">
                ...
                <Link
                  to="/login"
                  onClick={() => setMenuOpen(false)}
                  className="w-full text-center border border-outline-variant/30 text-on-surface/70 py-3 rounded-sm text-sm font-label tracking-widest uppercase hover:text-secondary-container hover:border-secondary-container/40 transition-colors"
                >
                  {t('nav.login')}
                </Link>
              </div>
            ...
            <button
              ...
              className="text-left text-on-surface/40 text-xs uppercase tracking-widest pt-2 hover:text-secondary-container transition-colors"
            >
